 ...
 * @name DiscordGPT
 * @version 1.0.0
 * @description Automatic message response generation
 * @author ariocp
 * @source https://github.com/ariocp/DiscordGPT
 */

const gptLogoSvg = `
  <svg width="24" xmlns="http://www.w3.org/2000/svg" viewBox="300 300 1806 1806">
...
const generateAnswerButton = `
  <button id="gpt" aria-haspopup="dialog" aria-label="Generate Answer" type="button" class="button-ejjZWC lookBlank-FgPMy6 colorBrand-2M3O3N grow-2T4nbg">
...
const getLastMessage = () => {
  const messages = document.querySelectorAll('.markup-eYLPri.messageContent-2t3eCI');
  const lastMessage = messages[messages.length - 1];

  if (!lastMessage.parentElement.childNodes[1]) return;
  const content = lastMessage.innerText;
  return content;
};
...
async function getChat() {
  const chatId = window.location.href.split('/')[5];
...
async function generateAnswer() {
  const apiKey = apiKeys[id % apiKeys.length];
  id++;
  const prompt = await getLastMessage();
...
function addTextArea() {
  const element = document.querySelector('.channelTextArea-1FufC0.channelTextArea-1VQBuV');
  console.log(element);
  ...
  const crossBtn = createElementFromString(crossSvg);
...
async function sendMessage() {
  const channel = window.location.href.split('/')[5];
...
module.exports = class AnswerGpt {
  observer() {
    ...
    if (!gpt) {
      ...
      let gptButton = createElementFromString(generateAnswerButton);
      ...
      gpt.addEventListener("click", async () => {
        ...
        if (textarea) return;
        BdApi.showToast("Response generation has started...", { type: "info" });
